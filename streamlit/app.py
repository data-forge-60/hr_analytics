...
# --- 5. UI COMPONENTS ---
st.title("🚀 HR Attrition Pro (Model Based)")
...
st.sidebar.divider()
st.sidebar.header("2. Model Calibration")
threshold = st.sidebar.slider("Sensitivity Batas (Threshold)", 0.1, 0.9, 0.5, 0.05, help="Lower = Catch more potential resignees")
    ...
    if st.button("🔍 RUN ANALYTICS"):
        with st.spinner("Processing Model Predictions..."):
            try:
                ...
                m3.metric("Critical High Risk", f"{len(results[results['Probability'] >= 0.7]):,}")
                ...
                st.info("Employees listed here have over 70% probability of resignation. Immediate HR review recommended.")
                priority_list = results[results['Probability'] >= 0.7].sort_values('Probability', ascending=False)
